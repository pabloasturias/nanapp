...
TRANSLATIONS_FILE = './services/translations.ts'
...
translations = {
  ...
  "it": {
    ...
    "wearing_ergo_title": "Babywearing Ergonomico (Postura corretta)",
...
for lang, keys_dict in translations.items():
    for key, val in keys_dict.items():
        # Match `    key: "something",\n`
        # Because we know we inserted the english string identically
        key_clean = key.strip().replace(':', '')
        pattern = r'\s+' + key_clean + r':\s*".*?",?\n'
        replacement = f'\n    {key_clean}: "{val}",\n'
        
        # Only replace inside the specific language block
        lang_start = content.find(f'  {lang}: {{')
        lang_end = content.find('  },', lang_start)
        if lang_end == -1: lang_end = len(content)
        
        section = content[lang_start:lang_end]
        section = re.sub(pattern, replacement, section)
        content = content[:lang_start] + section + content[lang_end:]

with open(TRANSLATIONS_FILE, 'w', encoding='utf-8') as f:
    f.write(content)

print("Translated everything perfectly!")
