...
interface ControlsProps {
  ...
  hapticsEnabled: boolean; // NEW PROP
}
  ...
  return (
    <div className="flex flex-col gap-3 w-full bg-slate-900/80 backdrop-blur-xl p-4 rounded-[2.5rem] border border-orange-100/5 shadow-[0_-10px_40px_-10px_rgba(2,6,23,0.6)]">
      ...
      <div className="grid grid-cols-3 gap-3 mt-0">
          {/* Pause */}
          <button
            onClick={() => { onPause(); triggerHaptic(); }}
            disabled={!isPlaying}
            className={`flex flex-col items-center justify-center gap-1 h-14 rounded-[2rem] border transition-all active:scale-95
                ${!isPlaying && isPaused 
                    ? 'bg-orange-100/10 border-orange-200/20 text-orange-200' 
                    : 'bg-slate-800 border-slate-700/50 text-slate-400'}`}
          >
              <Pause size={20} fill="currentColor" className="opacity-80" />
              <span className="text-[9px] font-bold uppercase">{t('pause')}</span>
          </button>

          {/* Play */}
          <button
            onClick={() => { onPlay(); triggerHaptic(); }}
            className={`flex flex-col items-center justify-center gap-1 h-14 rounded-[2rem] border transition-all active:scale-95 shadow-xl
                ${isPlaying 
                    ? 'bg-orange-400/20 border-orange-400 text-orange-300 shadow-orange-500/10' 
                    : 'bg-orange-400 border-orange-300 text-white shadow-orange-500/20'}`}
          >
              <Play size={24} fill="currentColor" />
              <span className="text-[9px] font-bold uppercase">{isPlaying ? t('playing') : t('start')}</span>
          </button>

          {/* Stop / Finish */}
          <button
            onClick={() => { onStop(); triggerHaptic(); }}
            className="flex flex-col items-center justify-center gap-1 h-14 rounded-[2rem] bg-slate-800 border border-slate-700/50 text-slate-400 hover:text-red-300 hover:border-red-500/20 transition-all active:scale-95"
          >
